...
export const ProjectList: React.FC<Props> = ({ projects = [], onSelectProject, onCreateProject }) => {
  ...
  const getProjectGlobalAmount = (project: Project): number => {
    return parseAmount(project["Montant prévisionnel du marché (€ TTC)"] ?? project["Montant prévisionnel du marché"] ?? project["Montant"]);
  };

  const getProceduresTotalAmount = (project: Project): number => {
    if (!project.procedures || !Array.isArray(project.procedures)) return 0;
    return project.procedures.reduce((acc, proc) => acc + parseAmount(proc["Montant prévisionnel du marché (€ HT)"] ?? proc["Montant prévisionnel du marché"] ?? proc["Montant"]), 0);
  };
