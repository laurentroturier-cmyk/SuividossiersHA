import { fetchLeviersDaeTitles } from '../services/leviersDaeService';
...
export const ProjectDetail: React.FC<Props> = ({ project, allProjects, onBack, onSave, initialProcedureId }) => {
  ...
  const [leviersDaeOptions, setLeviersDaeOptions] = useState<string[]>([]);
  const [isLeviersDaeLoading, setIsLeviersDaeLoading] = useState(false);
  ...
  // Chargement des options "Levier DAE" depuis Supabase
  useEffect(() => {
    let cancelled = false;
    setIsLeviersDaeLoading(true);
    fetchLeviersDaeTitles()
      .then((options) => {
        if (!cancelled) setLeviersDaeOptions(options);
      })
      .catch(() => {
        if (!cancelled) setLeviersDaeOptions([]);
      })
      .finally(() => {
        if (!cancelled) setIsLeviersDaeLoading(false);
      });
    return () => {
      cancelled = true;
    };
  ...
  const getProceduresTotalAmount = () => {
    ...
    return formData.procedures.reduce((acc, proc) => acc + parseAmount(proc["Montant prévisionnel du marché"] ?? proc["Montant prévisionnel du marché (€ HT)"] ?? proc["Montant"]), 0);
  };
  ...
  const handleAddProcedure = async () => {
    ...
    const newProcedure: PurchaseProcedure = {
      ...
      "Type de procédure": "", "Code CPV Principal": "", "Montant prévisionnel du marché": "",
        ...
        <div className="p-8">
          {activeTab === 'general' && (
            <div className="animate-in fade-in slide-in-from-left-4 duration-300">
              ...
              {/* Titre du dossier sur une colonne entière */}
              ...
              {/* Les autres champs sur deux colonnes */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <SelectField label="Levier DAE" value={formData["Levier DAE"] || ""} onChange={(v) => setFormData(p => ({...p, "Levier DAE": v}))} options={leviersDaeOptions} isLoading={isLeviersDaeLoading} />
                <InputField 
                  label="Montant prévisionnel global (€)" 
                  value={formData["Montant prévisionnel du marché (€ TTC)"] ?? formData["Montant prévisionnel du marché"] ?? formData["Montant"]} 
                  onChange={(v) => setFormData(p => ({...p, "Montant prévisionnel du marché": v}))} 
            ...
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-300">
              <div className="flex justify-between items-center mb-6">
                <div className="flex flex-col gap-1">
                  <div className="flex items-center gap-2">
                    <div className="w-1 h-6 bg-primary rounded-full" />
                    <h3 className="text-lg font-semibold dark:text-white">Procédures du projet</h3>
                  </div>
                  <span className="text-xs font-bold text-primary dark:text-secondary">Total cumulé : {formatCurrency(formData.procedures?.reduce((acc, proc) => acc + (parseFloat(proc["Montant prévisionnel du marché"] || "0") || 0), 0))}</span>
                </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between mt-auto pt-4 border-t border-gray-100 dark:border-gray-700">
                        <div className="text-xs font-black text-primary dark:text-secondary">{formatCurrency(proc["Montant prévisionnel du marché"] ?? proc["Montant prévisionnel du marché (€ HT)"] ?? proc["Montant"])} </div>
                ...
                >
                   ...
                   <div className="space-y-10">
                       <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                          ...
                          <InputField label="Montant (€)" value={activeProcedure["Montant prévisionnel du marché"] ?? ""} onChange={(v: string) => handleUpdateProcedure(activeProcedure.id, "Montant prévisionnel du marché", v)} />
                       ...
                       <div className="bg-white/50 dark:bg-gray-800/30 p-6 rounded-2xl border dark:border-gray-700">
                          <h5 className="text-sm font-bold text-primary dark:text-secondary mb-4 flex items-center gap-2"><ShieldCheck size={16} /> Informations DAE</h5>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                              <SelectField label="Clauses sociales" value={activeProcedure["Dispo sociales"]} onChange={(v) => handleUpdateProcedure(activeProcedure.id, "Dispo sociales", v)} options={yesNoOptions} />
                              <SelectField label="Clauses environnementales" value={activeProcedure["Dispo environnementales"]} onChange={(v) => handleUpdateProcedure(activeProcedure.id, "Dispo environnementales", v)} options={yesNoOptions} />
