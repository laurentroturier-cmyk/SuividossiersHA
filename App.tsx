...
const App: React.FC = () => {
  ...
  const [appState, setAppState] = useState<'loading' | 'empty' | 'projets' | 'detail' | 'procedures' | 'export' | 'indicateurs' | 'outilsia' | 'error'>('loading');
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors">
      <header className="bg-white dark:bg-gray-800 shadow-sm border-b dark:border-gray-700 sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-8">
            <div className="flex items-center space-x-3 cursor-pointer" onClick={() => { setAppState('projets'); setSelectedProcedureId(null); }}>
              <div className="bg-white p-1.5 rounded-lg shadow-sm border border-gray-100 flex items-center justify-center">
                <img src="/logo-gestprojet.png" alt="Logo GestProjet" className="h-10 w-auto" />
              </div>
              <span className="text-xl font-bold text-primary dark:text-secondary">GestProjet</span>
            </div>
            {appState !== 'empty' && (
              <nav className="flex space-x-4">
                 <button onClick={() => { setAppState('projets'); setSelectedProcedureId(null); }} className={`px-3 py-2 rounded text-sm font-bold ${appState === 'projets' ? 'text-primary' : 'text-gray-400'}`}>Projets</button>
                 ...
                 <button onClick={() => { setAppState('indicateurs'); setSelectedProcedureId(null); }} className={`px-3 py-2 rounded text-sm font-bold ${appState === 'indicateurs' ? 'text-primary' : 'text-gray-400'}`}>Indicateurs</button>
                 <button onClick={() => { setAppState('outilsia'); setSelectedProcedureId(null); }} className={`px-3 py-2 rounded text-sm font-bold ${appState === 'outilsia' ? 'text-primary' : 'text-gray-400'}`}>Outils IA</button>
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        ...
        {appState === 'projets' && <ProjectList projects={projects} onSelectProject={(p) => { setSelectedProject(p); setSelectedProcedureId(null); setAppState('detail'); }} onCreateProject={() => { setSelectedProject({ ID: String(projects.length + 1), "Titre du dossier": "Nouveau Projet", procedures: [] } as any); setSelectedProcedureId(null); setAppState('detail'); }} />}
        {appState === 'detail' && selectedProject && <ProjectDetail project={selectedProject} allProjects={projects} onBack={() => { setAppState('projets'); setSelectedProcedureId(null); }} onSave={handleSaveProject} initialProcedureId={selectedProcedureId} />}
        ...
        {appState === 'indicateurs' && <IndicatorsPage projects={projects} />}
        {appState === 'outilsia' && <OutilsIA />}
